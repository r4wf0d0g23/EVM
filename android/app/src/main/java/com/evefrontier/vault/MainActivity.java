...
public class MainActivity extends BridgeActivity {
    ...
    private static final String AUTH_INTERCEPTOR_JS =
        ...
        + "      console.log('[EVM] Intercepted fetch (silent): ' + url);"
        + "      if (url.indexOf('.well-known/openid-configuration') !== -1) {"
        + "        return Promise.resolve(new Response(JSON.stringify(MOCK_META), {"
        + "          status: 200, headers: {'Content-Type': 'application/json'}"
        + "        }));"
        + "      }"
        // Hang other auth fetches silently — do NOT call requestLogin() here
        // to avoid loop when EVE Vault makes background auth requests after login
        ...
        + "      console.log('[EVM] Intercepted XHR (silent): ' + url);"
