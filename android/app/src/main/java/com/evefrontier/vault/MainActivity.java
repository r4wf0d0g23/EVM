...
public class MainActivity extends BridgeActivity {
    ...
    private static final String AUTH_INTERCEPTOR_JS =
        ...
        + "      });"
        + "    }"
        // Log ALL Enoki responses
        + "    if (url.indexOf('enoki.mystenlabs.com') !== -1) {"
        + "      console.log('[EVM] Enoki call: ' + url.substring(0,60));"
        + "      return _fetch.call(this, resource, options).then(function(r) {"
        + "        var c=r.clone(); c.text().then(function(t){"
        + "          console.log('[EVM] Enoki resp status=' + r.status + ' body=' + t.substring(0,300));"
        + "        }); return r;"
        + "      });"
        + "    }"
